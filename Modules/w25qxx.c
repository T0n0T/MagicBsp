...
void W25QXX_Read(uint8_t *pBuffer, uint32_t ReadAddr, uint16_t NumByteToRead)
{
    ...
    while (LL_SPI_IsActiveFlag_BSY(SPI_CH) && count) {
        count--;
    }
...
#else
    while (LL_SPI_IsActiveFlag_BSY(SPI_CH))
        ;
    while (!LL_SPI_IsActiveFlag_TXE(SPI_CH))
        ;
    LL_SPI_TransmitData8(SPI_CH, TxData);

    while (!LL_SPI_IsActiveFlag_RXNE(SPI_CH))
        ;
    Rxdata          = LL_SPI_ReceiveData8(SPI_CH);
...
void SPIx_ReadByte(uint8_t *pBuffer, uint16_t NumByteToRead)
{
    ...
    while (NumByteToRead--) {
        *rx_buf = SPIx_ReadWriteByte(0xff);
        rx_buf++;
    }
#endif
}

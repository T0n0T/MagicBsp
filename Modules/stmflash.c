...
void stmflash_write_2bytes(uint32_t waddr, uint32_t *pbuf, uint16_t length)
{
    ...
    {
        uint8_t index        = 0U;
        uint8_t nbiterations = 2U;
        while (waddr < endaddr) /* 写数据 */
        {
            for (index = 0U; index < nbiterations; index++) {
                if (stmflash_write_halfword(waddr + (2U * index), (uint16_t)(*pbuf >> (16U * index)))) /* 写入数据(每次2个字节) */
                {
                    break; /* 写入异常 */
                }
            }

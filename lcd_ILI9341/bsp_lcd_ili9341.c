...
void W25qxx_ReadGBK(rt_uint8_t *typeface, rt_uint8_t size, rt_uint8_t *dataBuf)
{
    rt_uint8_t qh, ql;
    rt_uint32_t foffset;
    rt_uint8_t csize = (size / 8 + ((size % 8) ? 1 : 0)) * (size); // 计算汉字点阵大小，单位字节数
    ...
    // 按字体的不同，在不同字库读取字体点阵
    case 12:
        err = sfud_read(flash, foffset + GBK_STORAGE_ADDR, csize, dataBuf);                // 12号字体
        break;
    case 16:
        err = sfud_read(flash, foffset + GBK_STORAGE_ADDR + 0x0008c460, csize, dataBuf);   // 16号字体
        break;
    case 24:
        err = sfud_read(flash, foffset + GBK_STORAGE_ADDR + 0x001474E0, csize, dataBuf);   // 24号字体
        break;
    case 32:
        err = sfud_read(flash, foffset + GBK_STORAGE_ADDR + 0x002EC200, csize, dataBuf);   // 32号字体
        break;
    }
    if (err != SFUD_SUCCESS)
    {
 ...
 ** 函数声明
 ****************************************************************************/
void sendOrder(rt_uint16_t data);                                                   // 向LCD发送：寄存器地址值(指令), 8位
void sendDataShort(rt_uint16_t data);                                               // 向LCD发送：数据，8位
rt_uint16_t readData(void);                                                        // 读取LCD返回的数据
void sendShort(rt_uint16_t Data);                                                   // 向液晶屏发送：数据，16位
void setCursor(rt_uint16_t xStart, rt_uint16_t yStart, rt_uint16_t xEnd, rt_uint16_t yEnd);  // 设置显示区域
...
void LCD_Init(void)
{
    ...
    if (xLCD.id != 0x9341)
    {
        LOG_E("check lcd...   %x", xLCD.id);        // 打印LCD ID
        return;                                     // 9341初始化失败
    }
 ...
 * 函数名： drawAscii
 * 功  能： 在指定位置显示一个字符
 * 参  数： rt_uint16_t x,y     起始坐标
 *          rt_uint8_t  num     要显示的字符:" "--->"~"
 *          rt_uint8_t  size    字体大小 12/16/24/32
 *          rt_uint32_t fColor  字体颜色
 *          rt_uint32_t bColor  背景颜色
 ...
 * 参  数： rt_uint16_t   x      字体左上角X坐标
 *          rt_uint16_t   y      字体左上角y坐标
 *          char* pFont  要显示的字符串数据
 *          rt_uint8_t    size   字号大小：12 16 24 32
 *          rt_uint32_t   fColor 字体颜色
 *          rt_uint32_t   bColor 背景颜色
 * 返回值:  无
 ...
 * 参  数： rt_uint16_t x,y     左上角起始坐标
 *          rt_uint16_t width   图片宽度
 *          rt_uint16_t height  图片高度
 *          rt_uint8_t* image   数据缓存地址
 *****************************************************************/
void LCD_Image(rt_uint16_t x, rt_uint16_t y, rt_uint16_t width, rt_uint16_t height, const rt_uint8_t *image)
{
    ...
    for (rt_uint16_t i = 0; i < height; i++)          // 一行一行地显示
    {
        setCursor(x, y + i, x + width, y + height); // 重新设置光标位置
        for (rt_uint16_t j = 0; j < width; j++)        // 一行中，从左到事，逐个像素处理
        {
 ...
 * 参  数： rt_uint16_t x  　   十字线的中心点坐标x
 *          rt_uint16_t y  　   十字线的中心点坐标x
 *          rt_uint16_t len     十字线的像素长度
 *          rt_uint32_t fColor  颜色
 * 返　回： 无
 * 备  注：
 *****************************************************************/
